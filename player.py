...
class Player:
    ...
    def think(self, mode, box_lists, agent_position, velocity):
        boxes = []
        for item in box_lists:
            if item.x - agent_position[0] > 0:
                boxes.append((item.x - agent_position[0], item.gap_mid - agent_position[1]))

        boxes.sort(key=lambda x: x[0])
        if boxes:
            first_near = boxes.pop(0)
        else:
            first_near = CONFIG['WIDTH'], CONFIG['HEIGHT'] / 2
        if boxes:
            second_near = boxes.pop(0)
        else:
            second_near = CONFIG['WIDTH'], CONFIG['HEIGHT'] / 2
        ...
        a = [agent_position[1] / CONFIG['HEIGHT'], velocity, first_near[0] / CONFIG['WIDTH'],
             first_near[1] / CONFIG['HEIGHT'],
             second_near[0] / CONFIG['WIDTH'], second_near[1] / CONFIG['HEIGHT'],
             (first_near[0] / CONFIG['WIDTH']) ** 2 + (first_near[1] / CONFIG['HEIGHT']) ** 2 * dir1,
             (second_near[0] / CONFIG['WIDTH']) ** 2 + (second_near[1] / CONFIG['HEIGHT']) ** 2 * dir2]
        self.nn.forward(a)
        if self.nn.y > 0.5:
            return 1
        else:
